...
>
> **Known gap blocking real-product MACROS in the Plan — UNIT CONVERSION.** Seed recipe lines use cups/tbsp/tsp and the seed products carry recipe-matched serving units (ratio=1), so `nutrition × qty/serving_size` works. Engine products are per-100g, so a "1 cup" line against a per-100g product computes 1/100th. Until a unit→grams converter exists (FDC `foodPortions` gramWeight, or a per-ingredient density table), `use-plan-day` only computes from a product when its `serving_unit` matches the line unit, else falls back to the recipe's cached nutrition. The picker therefore records product **selection + diet/allergen** safely; wiring real macros in is the next milestone.
>
